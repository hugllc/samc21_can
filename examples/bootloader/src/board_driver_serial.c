...
void uart_write_uint(Sercom *sercom, uint32_t i)
{
    uint8_t s;
    int8_t index = 28;
    uart_write_byte(sercom, (uint8_t)'0');
    uart_write_byte(sercom, (uint8_t)'x');
    for (; index >= 0; index -= 4) {
        s = (uint8_t)((i >> index) & 0xF);
        if (s < 10) {
            s += (uint8_t)'0';
        } else {
            s += (uint8_t)'A' - 10;
        }
        uart_write_byte(sercom, s);
    }
    uart_write_byte(sercom, 13);
    uart_write_byte(sercom, 10);
}
